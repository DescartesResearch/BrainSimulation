	...
	const int *x_indices, const int *y_indices, const int num_timeseries_elements)
{
    ...
    for (int i = 0; i < num_oberservationnodes; i++) {
		...
		series[i].timeseries = malloc(num_timeseries_elements * sizeof(t_nodeval));
...
void init_start_time_state(const int number_nodes_x, const int number_nodes_y, t_nodeval **nodes,
	const int num_start_levels, const t_nodeval *start_levels, const int *start_nodes_x, const int *start_nodes_y)
{
